...
public class ProjectionTest {
    ...
    /* 필기.
    *   [프로젝션(Projection]
    *   select 절 조회할 대상을 지정하는 것을 프로젝션이라 한다.
    *   (SELECT {프로젝션 대상}) FROM ...
    *   */
    ...
    *   2. 임베디드 타입 프로젝션
    *   - @임베드 가능한 -> 엔티티와 거의 비슷하게 사용되며 조희의 시작점이 될 수 없다.
    *   - 임베디드 타입은 영속 컨테스트에서 관리하지 않는다.
    *   3. 스칼라 타입 프로젝션
    ...
    *   - 역시 이 또한 영속 컨테스트에서 관리하지 않는다.  */
    ...
    @Test
    void testBiDirectionEntityProjection() {

        int menuCode = 7;

        BiDirectionCategory categoryOfMenu = service.biDrectionProjection(menuCode);
    ...
    void testEmbeddedProjection(){
        ...
        /* 우리가 조회를 할 건데, menuName 과 menuPrice 조회 */
